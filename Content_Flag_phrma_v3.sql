...
alter table public.mntr
add column flag_IPAB varchar(50);

update public.mntr
set flag_IPAB=
    case 
    when content_sans_https like '%independent payment advisory board%' then 'Yes'
else 'No'
    end;
...
set flag_medicare_part_d=
    ...
    when content_sans_https like '%negotiate medicare%' then  'Yes'
when content_sans_https like '%part d%' then  'Yes'
when content_sans_https like '%medicare negotiation%' then  'Yes'
...
set flag_medicare_part_b=
    ...
    when content_sans_https like '%part b%' then  'Yes'
